...
`;
(function() {
  const hash = document.querySelector(".page-footer-build code")?.textContent?.trim();
  if (!hash || hash === "unknown") return;
  for (const link of document.querySelectorAll('a[href="entropylab.html"]')) {
    link.download = `entropylab@${hash}.html`;
  }
})();
